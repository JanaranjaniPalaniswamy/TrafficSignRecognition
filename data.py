...
class PickledDataset(Dataset):
    """
    This is a class to read features and labels from input pascal file.
    """
...
class WrappedDataLoader:
    """
    This class batches the input dataset for a given batch size
    """
...
def extend_dataset(dataset):
    """
    The function to extend the datatset for balancing the unbalanced training datatset by flipping
    :param dataset: Imbalanced training dataset
    :return: Returns extended dataset
    """
    X = dataset.features
    y = dataset.labels
    num_classes = 43
...
def preprocess(path):
    """
    The function to convert the RGB images to Gray images in the train, valid and test datasets
    :param path: Dataset path in the pipeline
    :return: Gray datasets
    """
...
def get_train_loaders(path, device, batch_size, workers, class_count):
    """
    The function balances the imbalanced train dataset
    batches the training and validation sets and loads on the device
    :param path: dataset path
    :param device: Underlying Hardware accelerator to run the pipeline
    :param batch_size: size of each batch in the dataset
    :param workers: Number of subprocess to use for loading the data
    :param class_count: Number of samples in each class after extending and balancing
    :return: Extended and Balanced train and validation datasets loaded on to device
    """
    def to_device(x, y):
...
def get_test_loader(path, device, gray=True):
    """
    The function to real the data from test_gray.p file
    batches the read data and loads on to teh device
    :param path: Test dataset path
    :param device: Underlying hardware accelerator
    :param gray: Boolean variable indicating the dataset format (gray or RGB)
    :return: batched and loaded test dataset
    """
    def preprocess(x, y):
        return x.to(device), y.to(device, dtype=torch.int64)

...
def valid_batch(model, loss_func, x, y):
    """
    The function to validate correct predictions of the trained model over validation dataset
    :param model: Trained model
    :param loss_func: Loss function to calculate validation loss
    :param x: Features in validation set
    :param y: Labels in validation set
    :return: Validation loss per epoch, number of correct predictions and length of features
    """
    output = model(x)
...
def fit(epochs, model, loss_func, opt, train_dl, valid_dl, patience, checkpoint):
    """
    The function  train the model with train and validation set
    Calculates the training and validation losses and saves the model in checkpoint
    :param epochs: Number of training epochs
    :param model: Built model
    :param loss_func: Cross entropy loss
    :param opt: Adam optimizer
    :param train_dl: Extended training dataset batched and loaded onto device
    :param valid_dl: Batched and loaded validation dataset
    :param patience: Number of epoch to training until early stopping
    :param checkpoint: Checkpoint file to store the model during training
    :return: Function returns when the training is terminated due to early stopping.
    """
       wait = 0
